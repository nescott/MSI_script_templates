...
"${strain}_${ref}_${window}" "${in_bam}"

mean=$(zcat "${strain}_${ref}_${window}".regions.bed.gz \
| awk '{total += $4} END { print total/NR}')

zcat "${strain}_${ref}_${window}".regions.bed.gz \
| awk -v val="$mean" 'BEGIN{OFS="\t"} {print $1,$2,$3,$4/val}' \
> "${strain}_${ref}_${window}"_mosdepth.gg.tab

sed -f "sed_${ref}_chr" < "${strain}_${ref}_${window}"_mosdepth.gg.tab \
> "${strain}_${ref}_${window}"_chrnum_mosdepth.gg.tab
