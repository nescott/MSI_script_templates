...
pileup=open(sys.argv[1], "r")
my_file = pileup.readlines()
...
for i in my_file:  # process pileup file line by line.
    ...
    if len(line) > 4:
        ...
        reads_noIndels_noStartEnd = dump_indels(
            reads_noStartEnd
        )  # locus string without indels or end/start/quality.
        ...
        ref_count = len(re.findall("\.", reads_noIndels_noStartEnd)) + len(
            re.findall("\,", reads_noIndels_noStartEnd)
        )
    else:
        A = 0
        T = 0
    ...
    if (
        isA + isT + isG + isC > 1
    ):  # Only deal with loci where more than one base is seen.
        print(
            chrom
            + "\t"
            + pos
